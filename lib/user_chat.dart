...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> sendMessage() async {
    ...
    try {
      ...
      // ---> NEW: DECODE THE JSON TO CHECK FOR TOXICITY ERRORS <---
      final data = jsonDecode(res.body);

      if (data['status'] == 'error') {
        // 🚨 Show the red warning SnackBar if the AI blocks the message
        if (mounted) {
            ...
            SnackBar(
              content: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      data['msg'] ?? 'Message blocked due to community guidelines.',
              ...
              margin: const EdgeInsets.only(bottom: 80, left: 10, right: 10), // Keeps it above the text input
            ),
          );
        }
        // Clear the bad text so they don't try sending it again
        msgController.clear();

      } else if (data['status'] == 'ok') {
        // ✅ Message was safe, proceed as normal
        msgController.clear();
